...
def retokenize_and_load_langpair_dataset(
    ...
    os.system(f"python3 {BPE_DIR}/apply_bpe.py --dropout 0.1 -c {src_code_path} < {src_raw_path} > {src_tokenized_path}")
    os.system(f"python3 {BPE_DIR}/apply_bpe.py --dropout 0.1 -c {tgt_code_path} < {tgt_raw_path} > {tgt_tokenized_path}")

    os.system(f"fairseq-preprocess --source-lang {src} --target-lang {tgt} --srcdict {src_vocab_path} --tgtdict {tgt_vocab_path} --trainpref {os.path.join(data_path, 'train')} --destdir {data_path} --workers 20")
...
@dataclass
class RegularizationTranslationConfig(FairseqDataclass):
...
@register_task("translation-with-subword-regularization", dataclass=RegularizationTranslationConfig)
class TranslationWithSubwordRegularizationTask(FairseqTask):
    """
